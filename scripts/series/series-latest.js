...
window.onload = function getSeries(){
	...
	//Get the API.
	axios.get("https://api.themoviedb.org/3/tv/on_the_air?api_key="+API_KEY+'&language=pt-BR&page=1')
		.then((response)=>{
...
function search(pageNum){
	axios.get("https://api.themoviedb.org/3/tv/on_the_air?api_key="+API_KEY+'&language=pt-BR&page='+pageNum)
		.then((response)=>{
			let series = response.data.results;
			let output = "";
			for(let i = 0; i < series.length; i++){
