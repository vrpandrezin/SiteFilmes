...
window.onload = function getMovies(){
	...
	//Get the API.
	axios.get("https://api.themoviedb.org/3/movie/top_rated?api_key="+API_KEY+'&language=pt-BR&page=1')
		.then( (response) =>{
...
function search(pageNum){
	axios.get("https://api.themoviedb.org/3/movie/top_rated?api_key="+API_KEY+'&language=pt-BR&page='+pageNum)
		.then((response)=>{
			let movie = response.data.results;
			let output = "";
			for(let i = 0; i < movie.length; i++){
